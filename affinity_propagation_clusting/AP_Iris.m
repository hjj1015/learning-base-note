...
[A,B,C,D,Class] = textread('iris.data','%f%f%f%f%s','delimiter',',');
data=[A,B,C,D];
n = size(data,1);
%% 映射簇标签:C
CL = zeros(n,1);
CL(strcmp(Class, 'Iris-setosa')) = 1;
CL(strcmp(Class, 'Iris-versicolor')) = 2;
CL(strcmp(Class, 'Iris-virginica')) = 3;
%% plot(Irisdata):三维vs二维
figure(1)
plot(A(1:50),B(1:50),'ro'),grid on
hold on
plot(A(51:100),B(51:100),'go')
plot(A(101:150),B(101:150),'bo')
hold off
